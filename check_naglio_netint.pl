...
# ============================== SUMMARY =====================================
#
# Program : check_naglio_netint.pl
...
#
# Check if there are error increase of any of the interfaces, an is there any of the interfaces saturate
#
#  check_naglio_netint.pl -t 30 -H <IP> -l <USENAME> -x <authPassword> -X <privPassword> -L <authProtocol>,<privProtocol> -A \
...
#  [0.1  - Nov 8, 2013]  First beta verion check_naglio_netint.pl
#
...
my $o_port=161 ;               # Port for snmp connection
my $o_community= undef  ;      # Community for snmp connection
my $o_version2= undef;         # Snmp version         
my $o_login=undef ;            # login for snm v3 
my $o_passwd=undef ;           # snmp v3 auth password
my $o_privpass=undef ;         # snmp v3 privpass
my $v3protocols ;              # Protocol for snmp v3 
...
my $o_bulksnmp=     undef;     # SNMP Message size parameter (Makina Corpus contrib)
...
sub p_version { print "check_naglio_netint.pl version : $Version\n"; }
...
sub help {
   ...
   print "check_naglio_netint",$Version,"\n";
   print "by Marek Grzybowski - marek(at)grzybowski.waw.pl\n\n";
   print "This plugins gets 64 bit counters from device via SNMP,\n calculate rates and use power of naglio to check them.\n";
...
sub snmp_get_request {
  ...
  my $result = undef;
  my $result_subhash = undef;

  verb("Doing snmp request on ".$table_name." OIDs: ".join(' ',@{$oids_ref}));
  while (@{$oids_ref}){
     my @sublist = splice @{$oids_ref}, 0, 20; 
     verb("Doing snmp request on ".$table_name." OIDs chunk: ".join(' ',@sublist));
     $result_subhash = $session->get_request(
      -varbindlist => \@sublist ,
     );
     @{$result}{keys %{$result_subhash}} = values %{$result_subhash};
  }
  if (!defined($result)) {
...
'in_octet_64' , '1.3.6.1.2.1.31.1.1.1.6',
'in_ucast_64' , '1.3.6.1.2.1.31.1.1.1.7', 
'in_multicast_64' , '1.3.6.1.2.1.31.1.1.1.8',
'in_broadcast_64' , '1.3.6.1.2.1.31.1.1.1.9',
'in_error' , '1.3.6.1.2.1.2.2.1.14',
'in_discard' , '1.3.6.1.2.1.2.2.1.13',
'out_octet_64' , '1.3.6.1.2.1.31.1.1.1.10',
'out_ucast_64' , '1.3.6.1.2.1.31.1.1.1.11',
'out_multicast_64' , '1.3.6.1.2.1.31.1.1.1.12',
'out_broadcast_64' , '1.3.6.1.2.1.31.1.1.1.13',
'out_error' , '1.3.6.1.2.1.2.2.1.20',
'out_discard' , '1.3.6.1.2.1.2.2.1.19',
);

my $login = getlogin || getpwuid($<) || "Who";
my $save_file="/tmp/check_naglio_netint_".$login."_".$argv_str.'.perl';
   ...
   foreach my $table (keys %oid_tables) {
     my $snmp_result=undef ;
     my $timenow=time();
     #verb("Getting  Table ($table):". $oid_tables{$table});
      # Get description table
      $snmp_result = $session->get_table(
           -baseoid => $oid_tables{$table}
      );
     #print Dumper($snmp_result);  # entire snmp output
     ${$result{'1'}}{'timenow'}=$timenow;
     foreach my $key (keys %$snmp_result) {
        my $baseoid=$oid_tables{$table};
        $key =~ /$baseoid\.(.*)/;
        my $idx=$1;
        #$nlib->verb("Procesing inteface index:".$idx." Inteface value:".$snmp_result->{$key} ) ;
        ${$result{$idx}}{$table}=$snmp_result->{$key}; 
      }
    }
  ...
  foreach my $idx (keys %result){
     foreach my $table (keys %{$result{$idx}}) {
       my $rate=undef ;
       my $delta=undef ;
       my $name=undef;
       my $value=undef ;
       ...
       if  ( $table eq 'descr' ) {
         ...
         if (defined ${$result{$idx}}{'descr'} && defined ${$old_result{$idx}}{'descr'}) {
           $name=${$result{$idx}}{'descr'}."_".$table ;
           $value=${$result{$idx}}{$table};
           $old_value=${$old_result{$idx}}{$table};
           $delta=${$result{'1'}}{'timenow'} - ${$old_result{'1'}}{'timenow'};
           if (defined $value && defined $old_value &&  defined $delta ){
             $rate=($value-$old_value)/$delta;
             if ($rate >= 0 && ${$result{$idx}}{'descr'} =~ /$o_descr/  ) {
               $rate=sprintf "%.2f" , $rate ;
               if ( $table eq 'in_octet_64'  || $table eq 'out_octet_64' ){
                 $name .= "_rate_bps";
                 $nlib->verb("Adding new octet rate:".$name."=".$value);
                 $nlib->add_knownvar($name,$name,"GAUGE","");
                 $nlib->add_data($name,$rate*8);  # change octets to bits
               }else{
                 $name .= "_rate_ps";
                 $nlib->verb("Adding new rate:".$name."=".$value);
                 $nlib->add_knownvar($name,$name,"GAUGE","");
           ...
           #$nlib->add_data($name,$value);
             ...
             $nlib->verb("No descr for inteface idx:" . $idx);
         }
       }
     }
  }
